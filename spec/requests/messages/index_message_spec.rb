...
describe 'GET api/v1/conversations/:conversation_id/messages', type: :request do
  ...
  let!(:messages_user_1) { create_list :message, 5, user: user_1, conversation_id: conversation_id }
  let!(:messages_user_2) { create_list :message, 5, user: user_2, conversation_id: conversation_id }
  let!(:headers) { {} }
  let!(:params) { {} }
